...
public class UserOnboardingTests : IAsyncLifetime
{
    private readonly PostgreSqlContainer _postgres;
    private WebApplicationFactory<Program> _factory = null!;
    private HttpClient _client = null!;

    public UserOnboardingTests()
    {
    ...
    [Fact]
    public async Task CompleteOnboarding_WhenAuthenticated_ShouldReturn204()
    {
        var token = await RegisterAndLoginAsync("onboarding@orizonapp.io");
        _client.DefaultRequestHeaders.Authorization =
            new AuthenticationHeaderValue("Bearer", token);

        var response = await _client.PostAsJsonAsync(
            "/users/onboarding/complete", new { });

        response.StatusCode.Should().Be(HttpStatusCode.NoContent);
    }

    [Fact]
    public async Task CompleteOnboarding_WhenNotAuthenticated_ShouldReturn401()
    {
        var response = await _client.PostAsJsonAsync(
            "/users/onboarding/complete", new { });
    ...
    [Fact]
    public async Task CompleteOnboarding_WhenCompleted_ProfileShouldReflectChange()
    {
        var token = await RegisterAndLoginAsync("onboarding2@orizonapp.io");
        _client.DefaultRequestHeaders.Authorization =
            new AuthenticationHeaderValue("Bearer", token);

        await _client.PostAsJsonAsync("/users/onboarding/complete", new { });

        var profileResponse = await _client.GetAsync("/users/profile");
        var profile = await profileResponse.Content
            .ReadFromJsonAsync<System.Text.Json.JsonDocument>();

        profile!.RootElement
            .GetProperty("hasCompletedOnboarding")
            .GetBoolean()
            .Should().BeTrue();
    }
}